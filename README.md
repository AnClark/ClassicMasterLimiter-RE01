...
This repository includes a Windows cross-build Makefile. CPack handles packaging
for native builds and cross-builds.

### Native build for current platform

Configure and build the project, then create a package with:
...
The package is written to `build/` with a platform-specific name generated by
CPack.
...
The cross-build Makefile will report missing dependencies. Inspect the source
and build output for the exact packages required on your platform.
