...
void ClassicMasterLimiterUI::onImGuiDisplay()
{
    ...
    if (ImGui::Begin("Main Window", nullptr, kWindowFlags))
    {
                              ...
                              ImGuiWindowFlags_NoScrollWithMouse))
        {
            ...
            if (_BeginSection("THRESHOLD", SCALE(225.0f)))
            {
                ImGui::Dummy(ImVec2(SCALE(78.0f), 0.0f));
                ImGui::SameLine();
                _addThresholdKnob();
                _EndSection();
            }
            ...
            if (_BeginSection("COMPRESSION", SCALE(260.0f)))
            {
...

            {
                ...
                ImGui::Dummy(ImVec2(0, SCALE(2.0f)));
                _drawKjearhusLogo(ImVec2(SCALE(108.0f), SCALE(44.0f)));
                ImGui::Dummy(ImVec2(0, SCALE(28.0f)));

                const auto currentPos = ImGui::GetCursorScreenPos();
                ImGui::SetCursorScreenPos(ImVec2(currentPos.x - SCALE(60.0f), currentPos.y));
                _drawPluginName();
    ...
    if (fAboutWindowOpened)
    {
        ...
        if (ImGui::Begin("About Window", &fAboutWindowOpened, about_window_flags))
        {
            ...
            ImGui::SetColumnWidth(0, SCALE(520.0f));  // Left column width for content
            ImGui::SetColumnWidth(1, viewport->Size.x - SCALE(520.0f));  // Right column for button

            // Left column: content
            {
                // Add left padding
                ImGui::Indent(SCALE(20.0f));
            ...
            ImGui::NextColumn();
            {
                ...
                const ImVec2 button_size(SCALE(65.0f), SCALE(25.0f));
                const float buttonX = viewport->Pos.x + SCALE(520.0f) + (viewport->Size.x - SCALE(520.0f) - button_size.x) * 0.5f;  // Center in right column
                const float buttonY = viewport->Pos.y + viewport->Size.y - button_size.y - SCALE(10.0f);
